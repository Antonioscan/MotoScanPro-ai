...
Usa uno stile diretto, senza frasi introduttive, senza disclaimer e senza formule vaghe. Il testo deve essere perfettamente strutturato e pronto per la vendita.
    ...
    // 3. SE NON C'È IL PREZZO → CERCA SU ricambi-ducati.it
    if (!price && code && process.env.SERPAPI_API_KEY) {
      try {
        const serpResponse = await fetch(`https://serpapi.com/search.json?q=site:ricambi-ducati.it+${encodeURIComponent(code)}&engine=google&api_key=${process.env.SERPAPI_API_KEY}`);
        const serpData = await serpResponse.json();
        const firstResult = serpData.organic_results?.[0];

        if (firstResult?.snippet) {
          const priceMatch = firstResult.snippet.match(/([\d.,]+)\s?€/);
          if (priceMatch) {
            price = priceMatch[1].replace(',', '.').trim();
          }
        }
      } catch (serpError) {
        console.warn("Errore nella ricerca su ricambi-ducati.it:", serpError.message);
      }
    }

    // 4. SE ANCORA NON C'È IL PREZZO → fallback su eBay via SerpAPI
    if (!price && code && process.env.SERPAPI_API_KEY) {
      try {
        const serpResponse = await fetch(`https://serpapi.com/search.json?engine=ebay&ebay_domain=ebay.it&q=${encodeURIComponent(code)}&api_key=${process.env.SERPAPI_API_KEY}`);
        const serpData = await serpResponse.json();
        const result = serpData.organic_results?.find(item => item.price);

        if (result?.price) {
          price = result.price.replace(/[^\d.,]/g, '').replace(',', '.').trim();
        }
      } catch (serpError) {
        console.warn("Errore nella ricerca su eBay:", serpError.message);
      }
    }
